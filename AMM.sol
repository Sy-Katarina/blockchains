	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		if( sellToken == tokenA ) {
			qtyA = sellAmount;
			// dy = y - (k / (x + dx_eff))
			swapAmt = reserveB - (invariant / (reserveA + effectiveSell));
			require( swapAmt > 0 && swapAmt <= reserveB, 'Insufficient liquidity' );
			...
			qtyB = sellAmount;
			swapAmt = reserveA - (invariant / (reserveB + effectiveSell));
			require( swapAmt > 0 && swapAmt <= reserveA, 'Insufficient liquidity' );
